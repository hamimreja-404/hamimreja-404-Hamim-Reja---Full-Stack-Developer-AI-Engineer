        ...
        <div className="absolute left-0 top-1/2 -translate-y-1/2 animate-postman-fly">
          <div className="flex flex-col items-center">
            <div className="relative">
               <MailIcon size={40} style={{ color: colors.primary }} className="fill-current opacity-20" />
               <MailIcon size={40} style={{ color: colors.primary }} className="absolute inset-0" />
               <div className="absolute -right-2 -top-2 animate-ping h-3 w-3 rounded-full bg-sky-400 opacity-75"></div>
            </div>
            <span className="text-[10px] font-bold mt-2 uppercase tracking-tighter" style={{ color: colors.primary }}>Delivering...</span>
          </div>
        </div>
...
export default function BuyPlans() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // Handle server-side errors here (e.g., show a toast)
  ...
  if (isSent) {
    return (
      <div className="min-h-screen flex items-center justify-center p-6 transition-colors duration-300"
        ...
        <SEO title="Thank You" />
        <div className="max-w-md w-full text-center p-8 rounded-3xl border shadow-2xl animate-fade-in"
             style={{ backgroundColor: colors.surface, borderColor: colors.border }}>
          <div className="w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6 animate-bounce-short"
          ...
          <p className="mb-8 leading-relaxed" style={{ color: colors.textSecondary }}>
            I've received your request for the <span className="font-bold" style={{ color: colors.primary }}>{formData.plan}</span>. 
            I'll contact you via <span className="font-bold">{formData.mobile}</span> as soon as possible.
          </p>
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-6 transition-colors duration-300 relative overflow-hidden"
         style={{ backgroundColor: colors.background, color: colors.textPrimary }}>
      <SEO title={`Select ${formData.plan}`} />
      
      {/* Dynamic Background Blobs */}
      <div className="absolute top-0 right-0 w-96 h-96 rounded-full blur-[120px] -translate-y-1/2 translate-x-1/2 opacity-20"
           style={{ backgroundColor: colors.primary }}></div>
      <div className="absolute bottom-0 left-0 w-96 h-96 rounded-full blur-[120px] translate-y-1/2 -translate-x-1/2 opacity-20"
      ...
      <div className="relative w-full max-w-xl mt-4">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          >
            <div className="p-2 rounded-full border transition-all group-hover:bg-opacity-10"
                 style={{ borderColor: colors.border, backgroundColor: `${colors.textSecondary}10` }}>
              <ArrowLeft size={16} />
            </div>
            <span className="font-bold text-sm group-hover:text-current">Return</span>
          </button>

         
        </div>

        <div className="relative p-8 md:p-12 rounded-[2.5rem] border shadow-2xl overflow-hidden backdrop-blur-sm"
             style={{ backgroundColor: isDark ? `${colors.surface}cc` : `${colors.surface}f2`, borderColor: colors.border }}>
          
          <PostmanAnimation isVisible={showPostman} colors={colors} />

          <div className={`transition-all duration-500 ${isSubmitting ? 'opacity-20 blur-md pointer-events-none' : 'opacity-100'}`}>
            <div className="mb-10">
              <h1 className="text-3xl font-bold mb-2">Finalize Your Plan</h1>
              <p style={{ color: colors.textSecondary }}>Complete the details below to get started with your digital upgrade.</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="p-4 rounded-2xl border" style={{ backgroundColor: colors.background, borderColor: colors.border }}>
                  <label className="text-[10px] uppercase font-bold tracking-widest block mb-1" style={{ color: colors.primary }}>Plan Name</label>
                  <div className="flex items-center gap-2 font-bold">
                    <Briefcase size={14} className="opacity-50" />
                    <span className="truncate">{formData.plan}</span>
                  </div>
                </div>
                <div className="p-4 rounded-2xl border" style={{ backgroundColor: colors.background, borderColor: colors.border }}>
                  <label className="text-[10px] uppercase font-bold tracking-widest block mb-1" style={{ color: colors.secondary }}>Investment</label>
                  <div className="flex items-center gap-2 font-bold">
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  <label className="text-xs font-bold ml-1" style={{ color: colors.textSecondary }}>Full Name</label>
                  <div className="relative">
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold ml-1" style={{ color: colors.textSecondary }}>Mobile Number</label>
                  <div className="relative">
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold ml-1" style={{ color: colors.textSecondary }}>Email Address <span className="font-normal opacity-50">(Optional)</span></label>
                  <div className="relative">
